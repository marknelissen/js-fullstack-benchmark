...
var (
	mu       sync.Mutex
	posts    = []*Post{{ID: 1, Title: "Islands architecture", Likes: 3}, {ID: 2, Title: "htmx + Vue", Likes: 1}}
...
// commentsHandler renders the htmx fragment for a post's comment list + the Vue CommentForm island.
...
// likeHandler is called directly by the Vue LikeButton island via fetch (not htmx).
...
// addCommentHandler is called by the Vue CommentForm island via fetch, then tells htmx
